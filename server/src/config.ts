...
const GOOGLE_AUTH_CLIENT_IDS = (process.env.GOOGLE_AUTH_CLIENT_IDS ?? '')
  ...
  .filter(Boolean);
const CRAWLER_LOGIN_USERNAME = (process.env.CRAWLER_LOGIN_USERNAME ?? '').trim();
const CRAWLER_LOGIN_PASSWORD = (process.env.CRAWLER_LOGIN_PASSWORD ?? '').trim();
const CRAWLER_SESSION_SECRET = (process.env.CRAWLER_SESSION_SECRET ?? '').trim();
const CRAWLER_SESSION_MAX_AGE_SECONDS_RAW = Number(process.env.CRAWLER_SESSION_MAX_AGE_SECONDS ?? 86400);
const CRAWLER_SESSION_MAX_AGE_SECONDS = Number.isFinite(CRAWLER_SESSION_MAX_AGE_SECONDS_RAW) && CRAWLER_SESSION_MAX_AGE_SECONDS_RAW > 0
  ? Math.floor(CRAWLER_SESSION_MAX_AGE_SECONDS_RAW)
  : 86400;
const CRAWLER_SESSION_COOKIE_DOMAIN = (process.env.CRAWLER_SESSION_COOKIE_DOMAIN ?? '').trim();
